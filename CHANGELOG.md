...
### Added
- 账号新增取信状态分级标记，区分“取信失效”“取信失败”“正常”和“未检查”，并在刷新 token、手动取信、邮件转发轮询与详情拉取时自动更新。
- 账号列表新增可折叠的取信状态筛选入口，默认收起且视为“全部状态”；展开后可按正常、失效邮箱、取信失败和未检查筛选账号。
- 账号卡新增独立的取信状态标签与“查看取信错误”入口，可查看最近一次取信状态的来源、原因、时间和错误摘要。

### Changed
- `folder=all` 的普通邮箱取信失败结果会保留 inbox / junkemail 的内层 Graph、IMAP 和 OAuth 详细错误，前端错误弹窗不再只显示笼统失败摘要。
- 邮件内容搜索返回的命中账号列表会同步携带取信状态字段，保持与普通账号列表一致的状态展示。
...
### Added
- 邮件搜索模式新增可展开的“排除条件”，包含规则和排除规则都支持多行输入；账号只要命中任一包含规则且同范围内不命中任一排除规则才会返回，并支持只填写排除条件来筛选“不包含这些邮件”的账号。
- 邮件内容搜索范围新增发件人字段，并保持标题、预览和去 HTML 后正文的本地保留搜索能力。
- 邮件搜索未选账号时，右侧展示全部命中邮件；搜索结果中的“查看用户邮件”按钮会进入对应账号邮箱，并同步滚动左侧账号列表到该用户。

### Changed
- 邮件搜索命中账号列表继续沿用原账号卡 UI、多选流程和更多操作菜单，仅额外展示“X 封命中”。
- 邮件搜索刷新页面不再记住当前搜索 tab，默认回到账号搜索模式；切换账号/邮件搜索 tab 时会同步清理右侧邮件列表状态。
...
### Added
- 新增“账号 / 邮件”搜索模式切换；邮件模式会搜索本地保留数据库中的标题、预览和正文，左侧展示命中账号列表，右侧展示全部或指定账号的命中邮件，并支持点击同一账号取消筛选。
- 普通邮箱本地保留开启时，邮件转发轮询会把候选邮件和已取到的详情正文写入本地保留缓存；轮询缓存的新邮件在未被普通列表确认前保持隐藏，仍保留“点击显示”/“自动展示新邮件”逻辑，并支持被本地搜索命中。

### Fixed
- 加强邮件列表验证码识别规则，补充繁中、日文、韩文、意大利语、法语、阿语、西语、葡语和俄语验证码上下文，并支持全角数字、阿拉伯-印度数字和波斯数字归一化，修复日文 `一時検証コード` 等验证码邮件无法识别的问题。

## [2.0.63+2] - 2026-06-06

- 普通邮箱本地保留设置新增“自动展示新邮件”开关，默认关闭；开启后后台同步发现新邮件会自动合并到当前列表并保留高亮提示，不再等待点击“点击显示”。
- 邮件列表新增验证码识别与蓝色复制按钮，命中验证码时在标题右侧直接显示验证码，点击即可复制且不会打开邮件详情。
- 常规设置新增“开启复制验证码”开关，默认开启；关闭后前端不再识别验证码，也不渲染复制按钮。
