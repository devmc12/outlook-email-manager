...
- 取信状态筛选启用且搜索范围为“所有分组”时，普通账号列表请求不再携带 `group_id`，由后端按全库账号范围过滤状态。
- 保持“当前分组”范围的原有行为：选择当前分组时仍会携带当前 `group_id`，只过滤该分组及其子分组账号。
- 加载更多、清空搜索框、切换搜索范围等入口都会复用同一范围判断，避免局部刷新后退回当前分组。
- 跨所有分组显示取信状态筛选结果时，账号卡会展示所属分组，列表标题显示“所有分组”，减少范围误判。
- 补充后端和前端静态测试，覆盖不带 `group_id` 的状态筛选返回跨分组账号，以及前端所有分组筛选请求路径。
