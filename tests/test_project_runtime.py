...
class FrontendMailContentSearchTests(unittest.TestCase):
    def setUp(self):
        ...
        self.accounts_js = pathlib.Path(ROOT_DIR, 'static', 'js', 'index', '04-accounts.js').read_text(encoding='utf-8')
        self.emails_js = pathlib.Path(ROOT_DIR, 'static', 'js', 'index', '05-emails.js').read_text(encoding='utf-8')
    ...
    def test_search_mode_toggle_and_compact_scope_are_present(self):
        ...
        self.assertIn('id="mailSearchExcludePanel"', self.layout_html)
        self.assertIn('id="mailSearchExcludeToggle"', self.layout_html)
        self.assertIn('id="mailSearchExcludeInput"', self.layout_html)
        self.assertIn('placeholder="排除规则：标题 / 发件人 / 预览 / 正文"', self.layout_html)
        ...
        self.assertIn('onclick="toggleMailSearchExcludePanel()"', self.layout_html)
        self.assertIn('.search-mode-toggle {', account_css)
        self.assertIn('.mail-search-exclude {', account_css)
        self.assertIn('.mail-search-exclude.is-expanded .mail-search-exclude-input', account_css)
        self.assertIn('.search-container[data-search-mode="mail"] textarea.search-input', account_css)
    ...
    def test_mail_search_frontend_calls_local_mail_search_api(self):
        ...
        self.assertIn('function readStoredSearchMode()', self.groups_js)
        self.assertIn("return 'account';", self.groups_js)
        self.assertIn('resetSelectedAccountView();', self.groups_js)
        ...
        self.assertIn("params.set('exclude_q', excludeQuery);", self.groups_js)
        self.assertIn('function getMailSearchExcludeValue()', self.groups_js)
        self.assertIn('function hasMailSearchCriteria(query = getSearchInputValue())', self.groups_js)
        self.assertIn('if (!normalizedQuery && !excludeQuery)', self.groups_js)
        self.assertIn('function toggleMailSearchExcludePanel()', self.groups_js)
        self.assertIn('function handleMailSearchExcludeInput()', self.groups_js)
        self.assertIn("handleMailSearchExcludeInput();", self.core_js)
        self.assertIn("nextMode === 'mail' ? hasMailSearchCriteria(searchQuery) : searchQuery", self.groups_js)
        ...
        self.assertNotIn('outlook_search_mode', self.groups_js)
    ...
    def test_mail_search_renders_account_hits_and_toggles_active_account(self):
        ...
        self.assertIn('data-account-email="${escapeHtml(acc.email)}"', self.groups_js)
        self.assertIn('class="account-item mail-search-account-hit ${isActive ? \'active\' : \'\'} ${account.status === \'inactive\' ? \'inactive\' : \'\'}"', self.groups_js)
        self.assertIn('class="account-select-checkbox"', self.groups_js)
        self.assertIn('data-refreshable="${accountType !== \'imap\' ? \'true\' : \'false\'}"', self.groups_js)
        self.assertIn('data-forward-enabled="${account.forward_enabled ? \'true\' : \'false\'}"', self.groups_js)
        self.assertIn('data-mail-search-account-id="${accountId}"', self.groups_js)
        self.assertIn('data-account-email="${escapeHtml(account.email || \'\')}"', self.groups_js)
        self.assertIn('onclick="handleMailSearchAccountItemClick(event, ${accountId})"', self.groups_js)
        self.assertIn('<button class="account-menu-trigger" type="button" data-account-menu-toggle="true" title="更多操作">⋯</button>', self.groups_js)
        self.assertIn('data-account-action="forwardingLogs"', self.groups_js)
        self.assertIn('data-account-action="edit"', self.groups_js)
        self.assertIn('${renderAccountGroupSummary(account, true)}', self.groups_js)
        self.assertIn('${renderAccountAliasSummary(account.aliases)}', self.groups_js)
        self.assertIn('${renderAccountFooter(account)}', self.groups_js)
        self.assertIn('<span class="mail-search-count-pill">${Number(account.match_count) || 0} 封命中</span>', self.groups_js)
        self.assertNotIn('mail-search-hit-icon', self.groups_js)
        self.assertNotIn('mail-search-latest', self.groups_js)
        self.assertIn('function handleMailSearchAccountItemClick(event, accountId)', self.groups_js)
        self.assertIn('handleAccountRowSelectionClick(event);', self.groups_js)
        self.assertIn('selectAccount(account.email);', self.groups_js)
        ...
        self.assertIn("const isMailSearchContext = typeof isMailSearchMode === 'function' && isMailSearchMode();", batch_js)
        self.assertIn('const isTempContext = !!isTempEmailGroup && !isMailSearchContext;', batch_js)

    def test_mail_search_shows_all_matching_emails_before_account_click(self):
        self.assertIn('function renderMailSearchEmailResults(data, emails, options = {})', self.groups_js)
        self.assertIn("updateEmailListHeader('匹配邮件', total || currentEmails.length);", self.groups_js)
        self.assertIn('renderEmailList(currentEmails);', self.groups_js)
        self.assertIn("renderMailSearchEmptyState('输入包含规则筛选用户')", self.groups_js)
        self.assertIn("return '包含规则：标题 / 发件人 / 预览 / 正文';", self.groups_js)
        self.assertIn('renderMailSearchEmailResults(data, nextEmails, { append });', self.groups_js)
    ...
    def test_mail_search_result_can_open_result_account_mailbox(self):
        email_css = pathlib.Path(ROOT_DIR, 'static', 'css', 'index', '05-email-content.css').read_text(encoding='utf-8')
        responsive_css = pathlib.Path(ROOT_DIR, 'static', 'css', 'index', '08-responsive.css').read_text(encoding='utf-8')

        self.assertIn('const viewAccountButton = isMailSearchList && email.account_email', self.emails_js)
        self.assertIn('class="email-view-account-btn"', self.emails_js)
        self.assertIn('data-account-email="${escapeHtml(email.account_email)}"', self.emails_js)
        self.assertIn('const recipientLine = recipientDisplayLabel || viewAccountButton', self.emails_js)
        self.assertIn('<div class="email-recipient-row">', self.emails_js)
        self.assertIn('email-recipient ${viewAccountButton ? \'email-recipient--with-action\' : \'\'}', self.emails_js)
        self.assertIn('<span class="email-recipient-text">${escapeHtml(recipientDisplayLabel)}</span>', self.emails_js)
        self.assertLess(self.emails_js.index('<div class="email-date">${formatDate(email.date)}</div>'), self.emails_js.index('${recipientLine}'))
        self.assertLess(self.emails_js.index('<div class="email-recipient-row">'), self.emails_js.index('${viewAccountButton}'))
        self.assertIn("event.target.closest('.email-view-account-btn[data-account-email]')", self.emails_js)
        self.assertIn("selectAccount(accountEmail);", self.emails_js)
        self.assertIn('function syncActiveAccountListItem(email, options = {})', self.accounts_js)
        self.assertIn('getAccountItemEmail(item) === normalizedEmail', self.accounts_js)
        self.assertIn("activeItem.scrollIntoView({ block: 'center', inline: 'nearest', behavior: options.behavior || 'smooth' });", self.accounts_js)
        self.assertIn('syncActiveAccountListItem(email);', self.accounts_js)
        self.assertIn('requestAnimationFrame(() => syncActiveAccountListItem(email));', self.accounts_js)
        self.assertIn('.email-recipient-row {', email_css)
        self.assertIn('.email-recipient--with-action {', email_css)
        self.assertIn('.email-recipient-text {', email_css)
        self.assertIn('.email-view-account-btn {', email_css)
        self.assertIn('.email-view-account-btn {', responsive_css)
